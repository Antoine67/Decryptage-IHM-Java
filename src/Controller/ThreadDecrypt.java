...
public class ThreadDecrypt extends Thread {
	...
	public ThreadDecrypt(String name, MultiThreading multiThreading, BigInteger beginValue, BigInteger wantedValue,
			String messageADecrypter, String clueAboutKey, int maxKeyLenght) {
		super(name);
		this.name = name;
	...
	public void run() {
		...
		/*
		 * while(beginValue != wantedValue && multiThreading.keyFinded == false &&
		 * !multiThreading.shouldStop) { beginValue++; } if(multiThreading.keyFinded ==
		 * false) {
		 * 
		 * }
		 */
	...
	public String letsDecrypt() {
		...
		/*
		 * String crypt =
		 * intArrayToString(controller.getModel().encrypt("bonjour je m appelle loic"
		 * ,"^^^^")); System.out.
		 * println("Exemple : 'bonjour je m'appelle Loic' avec une clé '^^^^' donne crypté :"
		 * + crypt );
		 * 
		 * int[] decrypt = controller.getModel().encrypt(crypt, "^^^^");
		 * System.out.println("Soit après decryptage :" +intArrayToString(decrypt));
		 * 
		 * 
		...
		for (BigInteger i = beginValue; wantedValue.compareTo(i) > 0; i = i.add(BigInteger.ONE)) {
			...
			/*
			 * while(temp_key.length()%8 != 0) { temp_key = '0' + temp_key; }
			 */

			// System.out.println(intArrayToString(controller.getModel().encrypt(messageADecrypter,
			// binaryToAscii(temp_key))) + " avec " +temp_key);

			// Decrypted data with current temp key
			// System.out.println(intArrayToString(controller.getModel().encrypt(messageADecrypter,
			// binaryToAscii(temp_key))));
			 ...
			 * intArrayToString(controller.getModel().encrypt(messageADecrypter,
			 * binaryToAscii(temp_key)))); return true; }
			 */
		...
		// All possibilities have been tried for a key of length 'MAX_KEY_LENGHT'
		// (default : 12)
		controller.setProgressBarState(false);
		return null;
	...
	/**
	 * Convert int array to String (ASCII) "50,50,50,62" to "222<"
	 * 
	 * @param array
	 * @return
	 */
	...
	/*
	 * public void hello(int a) throws InterruptedException { this.a = a;
	 * this.keyFinded = true; System.out.println(a);
	 * System.out.println("statut du thread " + thread1.getName() + " = " +
	 * thread1.getState()); System.out.println("statut du thread " +
	 * thread2.getName() + " = " + thread2.getState());
	 * System.out.println("statut du thread " + thread3.getName() + " = " +
	 * thread3.getState()); System.out.println("statut du thread " +
	 * thread4.getName() + " = " + thread4.getState()); }
	 */
	...
	private Boolean validateKey(String toValidate) {
		...
		for (int e = 0; e < toTry.length; e++) {
			if (!toTry[e].matches("^[a-z0-9]+$")) {
				return false;
			} // Only alpha numeric char, except majs "^[a-zA-Z0-9]+$"
